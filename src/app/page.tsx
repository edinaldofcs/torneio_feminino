...
export default function Page() {
  ...
  const [selecionados, setSelecionados] = useState<Jogador[]>(() => {
    if (typeof window !== "undefined") {
      try {
        const armazenados = localStorage.getItem("selecionados_feminino");
  ...
  const [duplas, setDuplas] = useState<Dupla[]>(() => {
    if (typeof window !== "undefined") {
      try {
        const armazenadas = localStorage.getItem("duplas_feminino");
  ...
  useEffect(() => {
    try {
      ...
      const armazenadasDuplas = localStorage.getItem("duplas_feminino");
  ...
  useEffect(() => {
    localStorage.setItem("selecionados_feminino", JSON.stringify(selecionados));
  }, [selecionados]);

  useEffect(() => {
    localStorage.setItem("duplas_feminino", JSON.stringify(duplas));
  }, [duplas]);
  ...
  async function gravarHistorico() {
    ...
    if (error) {
      ...
      localStorage.removeItem("duplas_feminino");
      localStorage.removeItem("selecionados_feminino");
